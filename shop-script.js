...
document.addEventListener('DOMContentLoaded', () => {
    const themeGalleryWindow = document.getElementById('themeGalleryWindow'); // New ID
    ...
    const themes = { /* ... (Keep your FULL themes object definition here, same as in script.js) ... */
        default: { name: "Default Retro", cost: 0, owned: true, description: "The classic look and feel.", cssVariables: { /* ... */ } },
        oceanDepths: { name: "Ocean Depths", cost: 1, description: "Dive into cool blue tranquility.", cssVariables: { /* ... */ } },
        volcanoRush: { name: "Volcano Rush", cost: 1, description: "Fiery reds and oranges.", cssVariables: { /* ... */ } },
        techOrangeBlue: { name: "Tech Orange & Blue", cost: 1, description: "A modern tech-inspired palette.", cssVariables: { /* ... */ } },
        forestGreens: { name: "Forest Greens", cost: 1, description: "Earthy and calming greens.", cssVariables: { /* ... */ } }
    };
     // Ensure full CSS variables are copied into each theme object above
    themes.default.cssVariables = { '--theme-primary-dark': '#264653', '--theme-primary-accent': '#2A9D8F', '--theme-secondary-accent': '#E9C46A', '--theme-tertiary-accent': '#F4A261', '--theme-highlight-accent': '#E76F51', '--theme-light-bg': '#EAEAEA', '--theme-card-bg': '#FFFFFF', '--theme-text-on-dark': '#EAEAEA', '--theme-page-bg': 'rgb(174, 217, 211)' };
    themes.oceanDepths.cssVariables = { '--theme-primary-dark': '#03045E', '--theme-primary-accent': '#0077B6', '--theme-secondary-accent': '#00B4D8', '--theme-tertiary-accent': '#90E0EF', '--theme-highlight-accent': '#CAF0F8', '--theme-light-bg': '#E0FBFC', '--theme-card-bg': '#FFFFFF', '--theme-text-on-dark': '#CAF0F8', '--theme-page-bg': '#ADE8F4' };
    themes.volcanoRush.cssVariables = { '--theme-primary-dark': '#2B0000', '--theme-primary-accent': '#6A0000', '--theme-secondary-accent': '#FF4500', '--theme-tertiary-accent': '#FF8C00', '--theme-highlight-accent': '#AE2012', '--theme-light-bg': '#FFF2E6', '--theme-card-bg': '#FFFFFF', '--theme-text-on-dark': '#FFDAB9', '--theme-page-bg': '#FFCDB2' };
    themes.techOrangeBlue.cssVariables = { '--theme-primary-dark': '#004C97', '--theme-primary-accent': '#4A7DB5', '--theme-secondary-accent': '#FF6600', '--theme-tertiary-accent': '#C0C0C0', '--theme-highlight-accent': '#FF7700', '--theme-light-bg': '#F0F0F0', '--theme-card-bg': '#FFFFFF', '--theme-text-on-dark': '#F0F0F0', '--theme-page-bg': '#E8E8E8' };
    themes.forestGreens.cssVariables = { '--theme-primary-dark': '#1A2B12', '--theme-primary-accent': '#335128', '--theme-secondary-accent': '#526F35', '--theme-tertiary-accent': '#8A9A5B', '--theme-highlight-accent': '#E0E7A3', '--theme-light-bg': '#F0F5E0', '--theme-card-bg': '#FFFFFF', '--theme-text-on-dark': '#E0E7A3', '--theme-page-bg': '#D8E0C0' };
    ...
    function showToast(message, duration = 2500) { /* ... same simple toast function ... */ }
    function applyThemeOnPage(themeId) { /* ... same applyThemeOnPage function ... */ }
    function saveShopData() { /* ... same saveShopData function ... */ }
    ...
    function renderShopPageThemes() {
        ...
        Object.entries(themes).forEach(([themeId, themeData]) => {
            ...
            if (currentTheme === themeId) {
                itemSlot.classList.add('highlighted-product'); // Highlight active theme
            }
            ...
            const button = itemSlot.querySelector('.vending-item-button');
            if (button) {
    ...
    function handleShopPageThemeAction(themeId, themeData, action, cardElement) {
        ...
        renderShopPageThemes(); // Update all buttons and statuses
    }

    // Particle effects for shop page (ensure these are defined)
    function createShopParticle(x, y, color, size, count, spread, speedMultiplier = 1) {
        if (!particleCanvasShop || !shopCtx) return;
        for (let i = 0; i < count; i++) {
            shopParticles.push({ x, y, size: Math.random() * size + 2, color, vx: (Math.random() - 0.5) * spread * speedMultiplier, vy: (Math.random() * -2.5 - 1) * speedMultiplier, life: 50 + Math.random() * 30 });
        }
